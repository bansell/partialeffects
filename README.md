...
Incidentally, the average age of participants is `r mean(experimental_data$age)`, so the intercept represents the average blood pressure for patients of this age at the end of the study _correcting for all other variables_ including drug treatment.
...
The entire `experimental_data_scaled` data is captured by the `mod_scaled` object created above. Therefore this function needs only a model object and the column name of the effect of interest:
```{r}
